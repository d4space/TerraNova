...
int Toy4NormStat()
{
  ...
  // W+ Muon
  double WpToMuNu_Yield[NBin]={25651.7, 14407.5, 8136.32, 6375.47, 3751.52,3991.11, 2352.82,2384.4,1450.12, 349.623, 111.385, 57.9785, 28.7645};
  double WpToMuNu_Yield_Error[NBin]={236.047, 133.515, 84.0563, 71.8006, 56.4416, 59.711, 49.6092, 49.0328, 39.4345, 19.0838, 11.7062, 10.5994, 6.96515};
  
  // W- Muon
  double WmToMuNu_Yield[NBin]={16758, 9735.23, 5624.22, 4392.25, 2687.33, 2866.47, 1709.4, 1726.86, 1048.68, 252.088, 83.7192, 39.5066, 17.8297};
  double WmToMuNu_Yield_Error[NBin]={192.633, 108.996, 71.5906, 59.348, 47.4341, 51.7885, 42.2154, 42.5309, 32.6359, 15.8024, 10.1541, 6.47734, 5.95479};

  // W+ Electron
  double WpToEleNu_Yield[NBin]={24631.1, 14167.5, 8330.13, 6667.14, 3974.45, 4134.48, 2370.23, 2368.65, 1490.21, 384.512, 121.627, 61.0698, 30.1186};
  double WpToEleNu_Yield_Error[NBin]={264.833, 149.708, 98.1539, 84.1393, 66.8583, 69.2939, 53.7141, 56.2531, 42.7481, 22.82, 12.2646, 8.73213, 7.36433};

  // W- Electron
  double WpToEleNu_Yield[NBin]={16370.5, 9598.88, 5667.44, 4496.62, 2710.76, 2880.66, 1684.13, 1759.39, 1129.6, 286.8, 88.4324, 42.8677, 17.7608};
  double WpToEleNu_Yield_Error[NBin]={214.862, 120.389, 81.3334, 67.7955, 54.8901, 59.6268, 45.3299, 46.4496, 37.862, 19.5144, 10.8293, 7.64301, 4.51305};

  double Yield[NBin];
  double Yield_Error[NBin];
  for(int i(0); i<NBin; i++)
  {
    Yield[i] = WpToMuNu_Yield[i] + WmToMuNu_Yield[i];
    Yield_Error[i] = TMath::Sqrt(WpToMuNu_Yield_Error[i]*WpToMuNu_Yield_Error[i] + WmToMuNu_Yield_Error[i]*WmToMuNu_Yield_Error[i]);
  }
