...
void NormToyStatErr(double *Xsec, double *Err, double *Norm_Xsec, double *Norm_Err);
...
int MakeRoot_WpT_FEWZ(TString BaseName)
{
  ...
  double Norm_Xsec_PDFp[12];
  double Norm_Xsec_PDFm[12];
  ...
  double NormDiff_Xsec_PDFp[12];
  double NormDiff_Xsec_PDFm[12];
  ...
  // Calculate Xsec_PDFp and PDFm 
  for(int i(0); i<12; i++)
  {
    Xsec_PDFp[i] = Xsec[i] + PDFErrP[i];
    Xsec_PDFm[i] = Xsec[i] - PDFErrM[i];
    printf("Xsec_PDFp : %.4f \t Xsec_PDFm : %.4f\n", Xsec_PDFp[i], Xsec_PDFm[i]);
  }

  // Calculate ScaleUp,Down case Total Xsec and Normalization
  for(int i(0); i<12; i++)
  {
    Total_Xsec_up += Xsec_up[i];
    Total_Xsec_down += Xsec_down[i];
    Total_Xsec_PDFp += Xsec_PDFp[i];
    Total_Xsec_PDFm += Xsec_PDFm[i];
  }
  ...
  for(int i(0); i<12; i++)
  {
    ...
    Norm_Xsec_PDFp[i] = Xsec_PDFp[i] / Total_Xsec_PDFp;
    Norm_Xsec_PDFm[i] = Xsec_PDFm[i] / Total_Xsec_PDFm;

  }

  // Cross-section and Statistical error Normalization by Toy method
  NormToyStatErr(Xsec,StatErr,Norm_Xsec,Norm_StatErr);

  // Print out All Normalized Xsec
  for(int i(0); i<12; i++)
  {
    //cout << "Norm_Xsec : " << Norm_Xsec[i] << "\t Norm_StatErr : " << Norm_StatErr[i] << endl; 
    printf("Norm_Xsec : %.8f \t +- %.8f \t ScaleUp : %.8f \t ScaleDown : %.8f \t PDFp : %.8f \t PDFm : %.8f\n",Norm_Xsec[i],Norm_StatErr[i],Norm_Xsec_up[i],Norm_Xsec_down[i],Norm_Xsec_PDFp[i], Norm_Xsec_PDFm[i]);
  }
  ...
  //for(int i(0); i<12; i++)
  //{
  //  printf("NormXsec : %f \t +- %.8f, \t %.4f %\n",Norm_Xsec[i],Norm_TotalUncer[i], Norm_TotalUncer[i]/Norm_Xsec[i]*100 );
  //}
  ...
  for(int i(0); i<12; i++)
  {
    ...
    NormDiff_Xsec_PDFp[i] = Norm_Xsec_PDFp[i] / BinWidth[i];
    NormDiff_Xsec_PDFm[i] = Norm_Xsec_PDFm[i] / BinWidth[i];
    ...
    NormDiff_PDFErr[i] = TMath::Max(fabs(NormDiff_Xsec_PDFp[i] - NormDiff_Xsec[i]),fabs(NormDiff_Xsec_PDFm[i] - NormDiff_Xsec[i])); 
    //NormDiff_TotalUncer[i] = Norm_TotalUncer[i] / BinWidth[i] ; 
    NormDiff_TotalUncer[i] = sqrt(NormDiff_StatErr[i]**2 + NormDiff_ScaleErr[i]**2 + NormDiff_PDFErr[i]**2) ; 
  ...
  //TH1D* hxsec_Norm = new TH1D("hxsec_Norm","hxsec_Norm",12,Wpt12Bins);
  TH1D* hxsec_NormDiff = new TH1D("hxsec_NormDiff","hxsec_NormDiff",12,Wpt12Bins);
  
  //TH1D* hStatErr_percent = new TH1D("StatErr_percent","StatErr_percent",12,Wpt12Bins); 
  //TH1D* hPDFErr_percent = new TH1D("PDFErr_percent","PDFErr_percent",12,Wpt12Bins); 
  //TH1D* hScaleErr_percent = new TH1D("ScaleErr_percent","ScaleErr_percent",12,Wpt12Bins);
...
void NormToyStatErr(double *_Xsec, double *_StatErr, double *_Norm_Xsec, double *_Norm_StatErr)
{
  int Ntoy = 100000;
  ...
  for(int e=0; e<Ntoy; ++e)
  {
    for(int i=0; i<12; ++i)
    {
      	temp[i]=gRandom->Gaus(_Xsec[i], _StatErr[i]);
    }
    double sum=0;
  ...
  for(int i=0; i<12; ++i)
  {
    _Norm_StatErr[i] = rms[i]; // return rms to Norm_StatErr
  }
  for(int i=0; i<12; ++i)
  {
