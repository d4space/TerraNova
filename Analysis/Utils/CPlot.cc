...
//--------------------------------------------------------------------------------------------------
void CPlot::AddHist1D(TH1D *h, TString drawopt, int color, int linesty, int fillsty, int mkerSty, int mkerSize)
{
  if(!h)
    return;
  ...
  h->SetMarkerStyle(mkerSty);
  h->SetMarkerSize(mkerSize);
...
{ 
  ...
  if(nHist2D>0) {
    for(unsigned i=0; i<fItems.size(); i++) {
      ...
      if(doSave) {
        gSystem->mkdir(sOutDirDummy,true);
        TString outname = sOutDirDummy+TString("/")+fName+TString(".");
...
//        pr->GetXaxis()->SetRangeUser(fXmin,fXmax);
  ...
  //  
  if(doSave) {
    gSystem->mkdir(CPlot::sOutDirDummy,true);
    //gSystem->mkdir(sOutDirDummy,true);
    TString outname = CPlot::sOutDirDummy+TString("/")+fName+TString(".");
    //TString outname = sOutDirDummy+TString("/")+fName+TString(".");
    if(format.CompareTo("all",TString::kIgnoreCase)==0) {
      c->SaveAs(outname+TString("png"));
      c->SaveAs(outname+TString("eps"));
