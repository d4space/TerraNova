...
void diffXSecResAcc_Powheg_Madgraph_ResBos(double intLumi_invpb  =   18.424,
                    int    mBins          =   30,  // Z mass
  ...
  */
  ...
  tl11->AddEntry(grPowheg,"Pythia/Powheg", "l");
  ...
  tl1->SetFillColor(0);
  tl1->SetBorderSize(0);
  tl1->AddEntry(grResbos,"Resbos", "l");
  ...
  tl2->SetFillColor(0);
  tl2->SetBorderSize(0);
  tl2->AddEntry(grMadgraph,"Madgraph", "l");
  ...
  TCanvas* ctot = new TCanvas("ctot","",800,0,800,600);
  ctot->SetFillColor(0);
  DrawWithRatioTotal(ctot, cTitle, gXSecWithSyst, 
				   ...
				   line111,  line11,   line12, 
				   tl11,     tl1,      tl2,
				   
				   pave, pave1,pave2);
 
     ctot->SaveAs("diffXSec_PowhegResBosMadgraph.png");
}
...
void DrawWithRatioTotal(TCanvas *canvas, char *cTitle,
  ...
  for (int iBin=0;iBin<gNum->GetN();iBin++) {
    ...
    Double_t x,yNum,yDen,yDen2,yDen3, error_high, error_low, error;
    
    gNum->GetPoint(iBin,x,yNum); //data
    
    gDen->GetPoint(iBin,x,yDen);
    gDen2->GetPoint(iBin,x,yDen2);
    gDen3->GetPoint(iBin,x,yDen3);
