...
void Zpt_ZmassCompPtBins_Gaus(const TString Mode,//Channel - Muon or Electron
    ...
    )
{
  ...
  //=============================
  for(int i(0);i<4;i++){
    sprintf(tmpName,"h1_ZmassMC_muPtP%d",i);
    if(corrName=="CorrTotalRegion")
      sprintf(tmpName,"h1_ZmassMCcorr_muPtP%d",i);
    sprintf(histName,"hMCptBinP_%d",i);
    hMCptBinP[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hMCptBinP[i]->Sumw2();
    sprintf(histName,"hRDptBinP_%d",i);
    
    sprintf(tmpName,"h1_ZmassData_muPtP%d",i);
    hRDptBinP[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hRDptBinP[i]->Sumw2();

    sprintf(tmpName,"h1_ZmassMC_muPtM%d",i);
    if(corrName=="CorrTotalRegion")
      sprintf(tmpName,"h1_ZmassMCcorr_muPtM%d",i);
    sprintf(histName,"hMCptBinM_%d",i);
    hMCptBinM[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hMCptBinM[i]->Sumw2();
    sprintf(histName,"hRDptBinM_%d",i);
    
    sprintf(tmpName,"h1_ZmassData_muPtM%d",i);
    hRDptBinM[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hRDptBinM[i]->Sumw2();
    ...
    plotTitle = "Zpt: " + mup_ptRange[i] + ", before correction";
    if(corrName=="CorrTotalRegion")
    {
      sprintf(tmpName,"pFit_ptBin%d_Corr",i);
      plotTitle = "Zpt: " + mup_ptRange[i] + ", after correction";
    }
    ...
    plotTitle = "Zpt: " + mum_ptRange[i] + ", before correction";
    if(corrName=="CorrTotalRegion")
    {
      sprintf(tmpName,"mFit_ptBin%d_Corr",i);
      plotTitle = "Zpt: " + mum_ptRange[i] + ", after correction";
    }
  ...
  //==================================
  //Zpt histograms: Set muon pt regions to the x-axis
  //==================================
  ...
  plotTitle = "Zpt: mean at p_{T}^{#mu^{+}}, before correction";
  if(corrName=="CorrTotalRegion")
    plotTitle = "Zpt: mean at p_{T}^{#mu^{+}}, after correction";
  ...
  plotTitle = "Zpt: width at p_{T}^{#mu^{+}}, before correction";
  if(corrName=="CorrTotalRegion")
    plotTitle = "Zpt: width at p_{T}^{#mu^{+}}, after correction";
