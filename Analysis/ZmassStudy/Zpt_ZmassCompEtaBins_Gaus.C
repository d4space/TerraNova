...
void Zpt_ZmassCompEtaBins_Gaus(const TString Mode,//Channel - Muon or Electron
    const TString corrName,
    const TString outputDir 
    )
{
  TString plotTitle;
  TString mu_etaRange[ScaleBins];
  ...
  for(int i(0);i<ScaleBins;i++){
    sprintf(tmpName,"h1_ZmassMC_muEtaP%d",i);
    if(corrName=="CorrTotalRegion")
      sprintf(tmpName,"h1_ZmassMCcorr_muEtaP%d",i);
    
    if(outputDir=="Zpt_ZmassPlotsEtaBins_Gaus_Bin12")
      sprintf(tmpName,"h1_ZmassMC_Trailing_muEtaP%d",i);

    sprintf(histName,"hMCetaBinP_%d",i);
    hMCetaBinP[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hMCetaBinP[i]->Sumw2();

    sprintf(tmpName,"h1_ZmassData_muEtaP%d",i);
    
    if(outputDir=="Zpt_ZmassPlotsEtaBins_Gaus_Bin12")
      sprintf(tmpName,"h1_ZmassData_Trailing_muEtaP%d",i);

    sprintf(histName,"hRDetaBinP_%d",i);
    hRDetaBinP[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hRDetaBinP[i]->Sumw2();

    sprintf(tmpName,"h1_ZmassMC_muEtaM%d",i);
    if(corrName=="CorrTotalRegion")
      sprintf(tmpName,"h1_ZmassMCcorr_muEtaM%d",i);
    
    if(outputDir=="Zpt_ZmassPlotsEtaBins_Gaus_Bin12")
      sprintf(tmpName,"h1_ZmassMC_Trailing_muEtaM%d",i);

    sprintf(histName,"hMCetaBinM_%d",i);
    hMCetaBinM[i]= (TH1D*)fname->Get(tmpName)->Clone(histName); hMCetaBinM[i]->Sumw2();
    
    sprintf(tmpName,"h1_ZmassData_muEtaM%d",i);
    
    if(outputDir=="Zpt_ZmassPlotsEtaBins_Gaus_Bin12")
      sprintf(tmpName,"h1_ZmassData_Trailing_muEtaM%d",i);
    ...
    plotTitle = "Zpt: " + mu_etaRange[i] + " of #mu^{+}, before correction";
    if(corrName=="CorrTotalRegion")
    {
      sprintf(tmpName,"pFit_etaBin%d_Corr",i);
      plotTitle = "Zpt: " + mu_etaRange[i] + " of #mu^{+}, after correction";
    }
    ...
    //plotMllEtaBinP->SetYRange(0.,1.4*(hRDetaBinP[i]->GetMaximum()));
    ...
    //plotMllEtaBinM->SetYRange(0.,1.4*TMath::Max(hRDetaBinM[i]->GetMaximum()));
  ...
  //==================================
  TString filename = outputDir + "/Zpt_MeanWidth_Gaus_" + corrName + ".root";
  ...
  plotTitle = "Zpt: mean at #eta of #mu^{+}, before correction";
  if(corrName=="CorrTotalRegion")
    plotTitle = "Zpt: mean at #eta of #mu^{+}, after correction";
  ...
  plotTitle = "Zpt: width at #eta of #mu^{+}, before correction";
  if(corrName=="CorrTotalRegion")
    plotTitle = "Zpt: width at #eta of #mu^{+}, after correction";
