...
#include "../Utils/const.h"
  ...
  double muBiasp[14] = {0};
  muBiasp[1 ] =0.8113     ;//0.9022;
  muBiasp[2 ] =0.6901     ;//0.6571;
  muBiasp[3 ] =0.6054     ;//1.1301;
  muBiasp[4 ] =0.6256     ;//3.3043;
  muBiasp[5 ] =0.6861     ;//2.7469;
  muBiasp[6 ] =0.8211     ;//3.0764;
  muBiasp[7 ] =0.9721     ;//3.2467;
  muBiasp[8 ] =1.1544     ;//5.5057;
  muBiasp[9 ] =1.3111     ;//5.3121;
  muBiasp[10] = 1.4246   ;//1.8476;
  muBiasp[11] = 1.5112   ;//1.6692;
  muBiasp[12] = 1.5678   ;//4.4869;
  muBiasp[13] = 1.5971   ;//5.9061;
 
  double muBiasm[14]={0};
  muBiasm[1 ] =0.3438  ;//0.4327;
  muBiasm[2 ] =0.4093  ;//0.7800;
  muBiasm[3 ] =0.5759  ;//0.3212;
  muBiasm[4 ] =0.8337  ;//2.2542;
  muBiasm[5 ] =1.0880  ;//2.3913;
  muBiasm[6 ] =1.3628  ;//3.4663;
  muBiasm[7 ] =1.6033  ;//3.1813;
  muBiasm[8 ] =1.8344  ;//4.2104;
  muBiasm[9 ] =2.0056  ;//3.7714;
  muBiasm[10] = 2.1103 ;//1.8951;
  muBiasm[11] = 2.1775 ;//2.5063;
  muBiasm[12] = 2.2153 ;//4.7858;
  muBiasm[13] = 2.2326 ;//6.1218;
 ...
 */ 
  ...
  {
    systtotalp[i] =sqrt(
	mutotaleffp[i] *mutotaleffp[i] 
	+mumetp[i] *mumetp[i] 
	+muMomResp[i] *muMomResp[i] 
    ...
    systtotalm[i] =sqrt(
	mutotaleffm[i] *mutotaleffm[i] 
	+mumetm[i] *mumetm[i] 
	+muMomResm[i] *muMomResm[i] 
...
//  Print out result
  ...
  TFile *fp = new TFile("../Unfolding/ResultWpToMuNu/Result_WpToMuNu.root");
  TFile *fm = new TFile("../Unfolding/ResultWmToMuNu/Result_WmToMuNu.root");
  TH1D *h_data_p;
  TH1D *h_data_m;
  TH1D *h_MC_p;
  TH1D *h_MC_m;
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muTrackSigErri[i] = sqrt(muTrackSigErrp[i]*muTrackSigErrp[i] + muTrackSigErrm[i]*muTrackSigErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muTrackBckErri[i] = sqrt(muTrackBckErrp[i]*muTrackBckErrp[i] + muTrackBckErrm[i]*muTrackBckErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muIdisoSigErri[i] = sqrt(muIdisoSigErrp[i]*muIdisoSigErrp[i] + muIdisoSigErrm[i]*muIdisoSigErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muIdisoBckErri[i] = sqrt(muIdisoBckErrp[i]*muIdisoBckErrp[i] + muIdisoBckErrm[i]*muIdisoBckErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muToyErri[i] = sqrt(muToyErrp[i]*muToyErrp[i] + muToyErrm[i]*muToyErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  cout <<"W+ POGErr \t W- POGErr \t W POGErr" <<endl;
  for(int i(1);i<14;i++)
  {
    muPOGErrp[i] = h_data_p->GetBinContent(i)*0.01*muPOGp[i];
    muPOGErrm[i] = h_data_m->GetBinContent(i)*0.01*muPOGm[i];
    muPOGErri[i] = sqrt(muPOGErrp[i]*muPOGErrp[i] + muPOGErrm[i]*muPOGErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muEffErri[i] = sqrt(muEffErrp[i]*muEffErrp[i] + muEffErrm[i]*muEffErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muScaleErri[i] = sqrt(muScaleErrp[i]*muScaleErrp[i] + muScaleErrm[i]*muScaleErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    //muMomResErri[i] = sqrt(muMomResErrp[i]*muMomResErrp[i] + muMomResErrm[i]*muMomResErrm[i]);
    muSmearErri[i] = sqrt(muSmearErrp[i]*muSmearErrp[i] + muSmearErrm[i]*muSmearErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    //muMomResErri[i] = sqrt(muMomResErrp[i]*muMomResErrp[i] + muMomResErrm[i]*muMomResErrm[i]);
    muMomResErri[i] = sqrt(muMomResErrp[i]*muMomResErrp[i] + muMomResErrm[i]*muMomResErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muMetErri[i] = sqrt(muMetErrp[i]*muMetErrp[i] + muMetErrm[i]*muMetErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muQCDBckErri[i] = sqrt(muQCDBckErrp[i]*muQCDBckErrp[i] + muQCDBckErrm[i]*muQCDBckErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muQCDShapeErri[i] = sqrt(muQCDShapeErrp[i]*muQCDShapeErrp[i] + muQCDShapeErrm[i]*muQCDShapeErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muEWKErri[i] = (muEWKErrp[i] + muEWKErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muSVDUnfErri[i] = sqrt(muSVDUnfErrp[i]*muSVDUnfErrp[i] + muSVDUnfErrm[i]*muSVDUnfErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muFSRErri[i] = (muFSRErrp[i] + muFSRErrm[i])  / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muLumiErri[i] = (muLumiErrp[i]+muLumiErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muUnfBiasErri[i] = sqrt(muUnfBiasErrp[i]*muUnfBiasErrp[i] + muUnfBiasErrm[i]*muUnfBiasErrm[i]) / WInclXsec[i] * 100; // % unit
  ...
  for(int i(1);i<14;i++)
  {
    ...
    muStatErri[i] = sqrt(muStatErrp[i]*muStatErrp[i] + muStatErrm[i]*muStatErrm[i]) / WInclXsec[i] * 100; // % unit
    cout << muStatErrp[i] << "\t" << muStatErrm[i] <<"\t" << muStatErri[i] <<endl;
  }
 
  double muTotalSysti[14]={0};
  cout <<" Wincl Total Systematics" <<endl;
  for(int i(1);i<14;i++)
  {
  ...
  double muPowhegPDFErrp[14]={0};
  double muPowhegPDFErrm[14]={0};
  double muPowhegPDFErri[14]={0};
  cout <<"W+ PowhegPDFErr \t W- PowhegPDFErr \t W PowhegPDFErr" <<endl;
  for(int i(1);i<14;i++)
  {
    muPowhegPDFErrp[i] = h_data_p->GetBinContent(i)*0.01*PowhegPDFp[i];
    muPowhegPDFErrm[i] = h_data_m->GetBinContent(i)*0.01*PowhegPDFm[i];
    muPowhegPDFErri[i] = (muPowhegPDFErrp[i] + muPowhegPDFErrm[i]) / WInclXsec[i] * 100; // % unit
    cout << muPowhegPDFErrp[i] << "\t" << muPowhegPDFErrm[i] <<"\t" << muPowhegPDFErri[i] <<endl;
  }
 
  TString resultDir = "WptXsecErrors";
  gSystem->mkdir(resultDir,kTRUE);

  TFile f_out(resultDir+"/WInclToMuNuErrors.root","recreate");
  ...
  ///Write all errors to root
    ...
    for(int i(1);i<14;i++)
    {
      ...
      h_TotalUncer->SetBinContent(i,muTotalUnceri[i]);
   
      h_PowhegPDF->SetBinContent(i,muPowhegPDFErri[i]);
