...
//void CalcZpT()
void MCToy_ZpT18BinPowhegStatErr()
{
  
  cout << fixed << setprecision(10);


  TFile* f_powheg = new TFile("./preFSR_Powheg.root");
  //TFile* f_madgraph = new TFile("./preFSR_Madgraph.root");
  ////TFile* f_madgraph = new TFile("./postFSR_Madgraph.root");

  TH1D* hPreFSR_Powheg = (TH1D*)f_powheg->Get("hPtMCAcc")->Clone("hPreFSR_Powheg");
  ...
  {
    cout << i+1 <<" bin Events : " << hPreFSR_Powheg->GetBinContent(i+1) << endl;
  }

  // Xsec = # of Events / LUMI
  for(int i(0); i<nbin; i++)
  {
    Xsec[i] = hPreFSR_Powheg->GetBinContent(i+1)  ;
    StatErr[i] = sqrt(hPreFSR_Powheg->GetBinContent(i+1))  ;
    cout << i+1 <<" bin Xsec : " << Xsec[i] << "\t stat erre : " << StatErr[i] << " , \t " << StatErr[i]/Xsec[i]*100 << " %" <<endl; // print out Xsec and stat error
  }
...
*/
  ...
  for(int i(0); i<nbin; i++)
  {
    cout << "NormStat : " << Norm_StatErr[i] << 
      //"\t NormPDF : " << Norm_PDFErr[i] <<
      endl;
  }
  ...
  for(int i(0); i<nbin; i++)
  {
    ...
    cout << i+1 <<" bin NormDiff_Xsec : " << NormDiff_Xsec[i] << "\t normDiffStatErr+- " <<  NormDiff_StatErr[i] <<  endl;
