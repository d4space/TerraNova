...
int theoryStudy_separate(const TString BaseName)
{
  ...
  for( int ipt(1);ipt<nBins;ipt++)
  {
    ...
    cout<<fixed << setprecision(2)<<"hData_TotalSystErr: "<<ipt<<"\t"<<hData_TotalSystErr->GetBinContent(ipt)<<endl;
  } 
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    ...
    cout<<scientific<< "Data NormDiff Xsec : " << hData_NormDiffXsec->GetBinContent(ipt) << "\t +- " << hData_NormDiffXsec->GetBinError(ipt)<< endl;
  }
  ...
  cout << "Resbos Xsec : " << ResBos_NormDiffXsec[ipt+1] << "\t error+ : " << Resb_errMax[ipt] << "\t error - : " << Resb_errMin[ipt] << endl; 
