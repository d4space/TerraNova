...
int theoryStudy_separate(const TString BaseName)
{
  ...
  if(BaseName== "WInclToMuNu")
  {
    ...
    {
      hData_TotalSystErr->SetBinContent(ipt,
	  sqrt(
      ...
      //cout << "hData_TotalSystErr : " << hData_TotalSystErr->GetBinContent(ipt) << endl;
    }
  } 
  ...
  {
    hData_TotalUncer->SetBinContent(ipt, 
	sqrt(
	  TMath::Power(hData_StatErr->GetBinContent(ipt),2)+
	  TMath::Power(hData_TotalSystErr->GetBinContent(ipt),2)));
    //cout<<"hData_TotalUncer: "<<ipt<<"\t"<<hData_TotalUncer->GetBinContent(ipt)<<endl; // % unit
  } 
  ...
  for( int ipt(1);ipt<nBins;ipt++)
  {
    ...
    //cout<<"hData_StatErr: "<<ipt<<"\t"<<hData_StatErr->GetBinContent(ipt)<<endl;
    //cout<<"hPowheg_PDFErr: "<<ipt<<"\t"<<hPowheg_PDFErr->GetBinContent(ipt)<<endl;
  } 
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    ...
    cout << "hData_Xsec_BornLogScale : " << hData_Xsec_BornLogScale->GetBinContent(ipt) << " +- " << hData_Xsec_BornLogScale->GetBinError(ipt) << endl;
  }
  ...
  hPowheg_Yield_BornAfterFidCut = (TH1D*)f_Data->Get("SVD_Born.Gen_Unweighted")->Clone("hPowheg_Yield_BornAfterFidCut");

  //Errors
  double Powheg_Xsec_BornStatErr[14];
  double Powheg_Xsec_BornPDFErr[14];
  double Powheg_Xsec_BornTotalErr[14];
  double Powheg_DiffXsec_BornStatErr[14];
  double Powheg_DiffXsec_BornPDFErr[14];
  double Powheg_DiffXsec_BornTotalErr[14];
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    Powheg_Xsec_BornStatErr[ipt] = hPowheg_Xsec_BornLinScale->GetBinContent(ipt) * sqrt(hPowheg_Yield_BornAfterFidCut->GetBinContent(ipt))/hPowheg_Yield_BornAfterFidCut->GetBinContent(ipt); // Stat Error : Xsec * sqrt(Yield)/Yield
    Powheg_Xsec_BornPDFErr[ipt] = hPowheg_Xsec_BornLinScale->GetBinContent(ipt) * hPowheg_PDFErr->GetBinContent(ipt) * 0.01; // convert PDF Error % -> number
    Powheg_Xsec_BornTotalErr[ipt] = sqrt(Powheg_Xsec_BornStatErr[ipt]*Powheg_Xsec_BornStatErr[ipt] + Powheg_Xsec_BornPDFErr[ipt]*Powheg_Xsec_BornPDFErr[ipt]);

    Powheg_DiffXsec_BornStatErr[ipt] =Powheg_Xsec_BornStatErr[ipt] / hWptBins_LinScale->GetXaxis()->GetBinWidth(ipt);
    Powheg_DiffXsec_BornPDFErr[ipt] = Powheg_Xsec_BornPDFErr[ipt] / hWptBins_LinScale->GetXaxis()->GetBinWidth(ipt); 
    Powheg_DiffXsec_BornTotalErr[ipt] = Powheg_Xsec_BornTotalErr[ipt] / hWptBins_LinScale->GetXaxis()->GetBinWidth(ipt);

    cout << "hPowheg_Xsec_BornLinScale : " << hPowheg_Xsec_BornLinScale->GetBinContent(ipt) << "\t StatErr : " << Powheg_Xsec_BornStatErr[ipt] << "\t PDFErr : " << Powheg_Xsec_BornPDFErr[ipt] << " \t TotalErr : " << Powheg_Xsec_BornTotalErr[ipt] << endl;
  }
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    ...
    hPowheg_Xsec_BornLogScale->SetBinError(ipt,Powheg_DiffXsec_BornTotalErr[ipt]);
    cout << "Powheg DiffXsec : " << hPowheg_Xsec_BornLogScale->GetBinContent(ipt) << " +- " << hPowheg_Xsec_BornLogScale->GetBinError(ipt) << endl;
  }
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    ...
    hRatioPowhegStatErrBand->SetBinError(ipt,Powheg_DiffXsec_BornStatErr[ipt]/hData_Xsec_BornLogScale->GetBinContent(ipt));
    ...
    hRatioPowhegPDFErrBand->SetBinError(ipt,(Powheg_DiffXsec_BornStatErr[ipt]/hData_Xsec_BornLogScale->GetBinContent(ipt))+(Powheg_DiffXsec_BornPDFErr[ipt]/hData_Xsec_BornLogScale->GetBinContent(ipt)));
    ...
    cout << "Ratio Powheg : " << hRatioPowhegStatErrBand->GetBinContent(ipt) << "\tStat Band : " << hRatioPowhegStatErrBand->GetBinError(ipt) << "\tTotal Band : " << hRatioPowhegPDFErrBand->GetBinError(ipt) << endl;
  }
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    ...
    cout << "FEWZ Xsec : " << hFEWZ_Xsec_LinScale->GetBinContent(ipt) << "\t PDF Err : " << FEWZ_Xsec_PDFErr[ipt] << "\t Scale Err : " << FEWZ_Xsec_ScaleErr[ipt] << "\t TotalUcer : " << FEWZ_Xsec_TotalErr[ipt] << endl;
  }
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    cout << "FEWZ Diff Scale : " << FEWZ_DiffXsec_ScaleErr[ipt] << "\t PDF" << FEWZ_DiffXsec_PDFErr[ipt] << endl;
  ...
  for( int ipt(0);ipt<nBins-1;ipt++)
  {
    ...
    cout << Form("ResbosXsec : %.8f \t error+ : %.8f \t error- : %.8f",hResBos30_CentralYield_LinScale->GetBinContent(ipt+1), Resb_errMax[ipt],Resb_errMin[ipt])<< endl;
  ...
  TPaveText *tb_New = new TPaveText(0.2,0.39,0.52,0.5,"NDC");
  tb_New->SetBorderSize(0);
  tb_New->SetFillStyle(0);
  tb_New->AddText("18.4 pb^{-1} at #sqrt{s} = 8 TeV");
  if (BaseName=="WInclToMuNu")
    tb_New->AddText("W #rightarrow #mu #nu");
  if (BaseName=="WInclToEleNu")
    tb_New->AddText("W #rightarrow e #bar{#nu}");
