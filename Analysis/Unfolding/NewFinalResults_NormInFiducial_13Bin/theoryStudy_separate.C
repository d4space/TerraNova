...
int theoryStudy_separate(const TString BaseName)
{
  ...
  for( int ipt(1);ipt<=nBins-1;ipt++)
  {
    Powheg_NormDiff_TotalErr[ipt] = sqrt(Powheg_NormDiff_StatErr[ipt]**2 + Powheg_NormDiff_PDFErr[ipt]**2);
  ...
  for(int i(1); i<14; i++)
  {
    FEWZ_NormDiff_TotalErr[i] = sqrt(FEWZ_NormDiff_StatErr[i]**2 + FEWZ_NormDiff_PDFErr[i]**2 + FEWZ_NormDiff_ScaleErr[i]**2);
  }
  ...
  ///   Theory/Data ratio plot errors related to Real Data
  TGraphAsymmErrors *Resb30_DiffXsec = new TGraphAsymmErrors(nBins-1, ax, hResb30_NormDiffXsec, aex, aex, Resb_errMin, Resb_errMax);
