...
int WMinusWplusRatio_logScale()
{
  ...
  const int nBins = 13;
  double WptBins_LinScale[nBins] = {0.0,7.5,12.5,17.5,30,40,50,70,110,150,190,250,600};
  double WptBins[nBins] = {1.,7.5,12.5,17.5,30,40,50,70,110,150,190,250,600};

  //Data 12 bin yield after fsr unfolding, to add Mu+  and Ele+, Mu- and Ele-
  TFile *f_WmToMu_yield = new TFile("Wpt_WmToMuNuNormDiffXsec_InFid_RDResBosPowhegFEWZ.root");
  TFile *f_WpToMu_yield = new TFile("Wpt_WpToMuNuNormDiffXsec_InFid_RDResBosPowhegFEWZ.root");
  TFile *f_WmToEle_yield = new TFile("Wpt_WmToEleNuNormDiffXsec_InFid_RDResBosPowhegFEWZ.root");
  TFile *f_WpToEle_yield = new TFile("Wpt_WpToEleNuNormDiffXsec_InFid_RDResBosPowhegFEWZ.root");
 
  TH1D* hWmToMu_RD_yield = (TH1D*)f_WmToMu_yield->Get("hData_Yield_BornLinScale12")->Clone("hWmToMu_RD_yield");
  TH1D* hWpToMu_RD_yield = (TH1D*)f_WpToMu_yield->Get("hData_Yield_BornLinScale12")->Clone("hWpToMu_RD_yield");
  TH1D* hWmToEle_RD_yield = (TH1D*)f_WmToEle_yield->Get("hData_Yield_BornLinScale12")->Clone("hWmToEle_RD_yield");
  TH1D* hWpToEle_RD_yield = (TH1D*)f_WpToEle_yield->Get("hData_Yield_BornLinScale12")->Clone("hWpToEle_RD_yield");

  cout<<"RD yields at PreFSR\t"<<endl;
  cout<<"bin \t Ele+ \t Ele- \t Mu+ \t Mu- "<<endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      hWpToEle_RD_yield->GetBinContent(i)<<"\t\t"<<
      hWmToEle_RD_yield->GetBinContent(i)<<"\t\t"<<
      hWpToMu_RD_yield->GetBinContent(i)<<"\t\t"<<
      hWmToMu_RD_yield->GetBinContent(i)<<"\t\t"<<
      endl;
  }

   TH1D* hWpToElePMuP_RD_yield;
   hWpToElePMuP_RD_yield = (TH1D*)hWpToEle_RD_yield->Clone("hWpToElePMuP_RD_yield");
   hWpToElePMuP_RD_yield->Add(hWpToMu_RD_yield);
   TH1D* hWmToEleMMuM_RD_yield;
   hWmToEleMMuM_RD_yield = (TH1D*)hWmToEle_RD_yield->Clone("hWmToEleMMuM_RD_yield");
   hWmToEleMMuM_RD_yield->Add(hWmToMu_RD_yield);

  cout<<"RD yields at PreFSR, adding Mu+  and Ele+, Mu- and Ele-\t"<<endl;
  cout<<"bin \t Ele+   +  Mu+   \t    Ele-  +   Mu- "<<endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      hWpToElePMuP_RD_yield->GetBinContent(i)<<"\t\t"<<
      hWmToEleMMuM_RD_yield->GetBinContent(i)<<"\t\t"<<
      endl;
  }

  TFile *f_EleMCCommonFid_ratio = new TFile("CommonFid_EleRatio_fromPowhegMC.root");
  TH1D* h_ElePRatioPreFSR = (TH1D*)f_EleMCCommonFid_ratio->Get("h1_ElePRatioPreFSR")->Clone("h_ElePRatioPreFSR");
  TH1D* h_EleMRatioPreFSR = (TH1D*)f_EleMCCommonFid_ratio->Get("h1_EleMRatioPreFSR")->Clone("h_EleMRatioPreFSR");
   TH1D* hWpToEleP_RD_yield_CommonFid;
   hWpToEleP_RD_yield_CommonFid = (TH1D*)hWpToEle_RD_yield->Clone("hWpToEleP_RD_yield_CommonFid");
   hWpToEleP_RD_yield_CommonFid->Multiply(h_ElePRatioPreFSR);
   TH1D* hWmToEleM_RD_yield_CommonFid;
   hWmToEleM_RD_yield_CommonFid = (TH1D*)hWmToEle_RD_yield->Clone("hWmToEleM_RD_yield_CommonFid");
   hWmToEleM_RD_yield_CommonFid->Multiply(h_EleMRatioPreFSR);



  cout<<"RD yields at PreFSR, apply commonFid ratio to Ele+ and Ele-\t"<<endl;
  cout<<"bin \t Ele+ \t ratio\t new Ele+ \t\t Ele- \t ratio\t new Ele-  "<<endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      hWpToEle_RD_yield->GetBinContent(i)<<"\t\t"<<
      h_ElePRatioPreFSR->GetBinContent(i)<<"\t\t"<<
      hWpToEleP_RD_yield_CommonFid->GetBinContent(i)<<"\t\t"<<
      hWmToEle_RD_yield->GetBinContent(i)<<"\t\t"<<
      h_EleMRatioPreFSR->GetBinContent(i)<<"\t\t"<<
      hWmToEleM_RD_yield_CommonFid->GetBinContent(i)<<"\t\t"<<
      endl;
  }
   TH1D* hWpToElePMuP_RD_yield_CommonFid;
   hWpToElePMuP_RD_yield_CommonFid = (TH1D*)hWpToEleP_RD_yield_CommonFid->Clone("hWpToElePMuP_RD_yield_CommonFid");
   hWpToElePMuP_RD_yield_CommonFid->Add(hWpToMu_RD_yield);
   TH1D* hWmToEleMMuM_RD_yield_CommonFid;
   hWmToEleMMuM_RD_yield_CommonFid = (TH1D*)hWmToEleM_RD_yield_CommonFid->Clone("hWmToEleMMuM_RD_yield_CommonFid");
   hWmToEleMMuM_RD_yield_CommonFid->Add(hWmToMu_RD_yield);

  cout<<"RD yields at Muon CommonFid at  PreFSR, adding Mu+  and Ele+, Mu- and Ele-\t"<<endl;
  cout<<"bin \t Ele+   +  Mu+   \t    Ele-  +   Mu- "<<endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      hWpToElePMuP_RD_yield_CommonFid->GetBinContent(i)<<"\t\t"<<
      hWmToEleMMuM_RD_yield_CommonFid->GetBinContent(i)<<"\t\t"<<
      endl;
  }


  //normalizing  (Ele+   +   Mu+)  diff Xec for Data
  TH1D *hWptBins_LinScale   = new TH1D("hWptBins_LinScale","hWptBins_LinScale",nBins-1,WptBins_LinScale); 
  TH1D *WPElePMuP_RD_ComFid_NormDiffXsec=new TH1D("WPElePMuP_RD_ComFid_NormDiffXsec","WPElePMuP_RD_ComFid_NormDiffXsec",nBins-1,WptBins_LinScale); 
  TH1D *WMEleMMuM_RD_ComFid_NormDiffXsec=new TH1D("WMEleMMuM_RD_ComFid_NormDiffXsec","WMEleMMuM_RD_ComFid_NormDiffXsec",nBins-1,WptBins_LinScale); 
  cout<<"normalizing  (Ele+   +   Mu+)  diff Xec for Data\t"<<endl;
  cout<<"bin \t (Ele+   +  Mu+)   \t    (Ele-  +   Mu-) "<<endl;
  cout << fixed << setprecision(10) << endl;
  for (int i(1); i<13 ; i++)
  {
    WPElePMuP_RD_ComFid_NormDiffXsec->SetBinContent(i,
	hWpToElePMuP_RD_yield_CommonFid->GetBinContent(i)/(hWpToElePMuP_RD_yield_CommonFid->Integral()*hWptBins_LinScale->GetXaxis()->GetBinWidth(i)));
    WMEleMMuM_RD_ComFid_NormDiffXsec->SetBinContent(i,
	hWmToEleMMuM_RD_yield_CommonFid->GetBinContent(i)/(hWmToEleMMuM_RD_yield_CommonFid->Integral()*hWptBins_LinScale->GetXaxis()->GetBinWidth(i)));
    
    
    cout<<i<<"\t"<<
      WPElePMuP_RD_ComFid_NormDiffXsec->GetBinContent(i)<<"\t\t"<<
      WMEleMMuM_RD_ComFid_NormDiffXsec->GetBinContent(i)<<"\t\t"<<
      endl;
  }

  cout<<"Total (Ele+   +  Mu+)   and   (Ele-  +   Mu-) "<<endl;
  cout<<hWpToElePMuP_RD_yield_CommonFid->Integral() <<endl;
  cout<<hWmToEleMMuM_RD_yield_CommonFid->Integral() <<endl;
  cout << fixed << setprecision(3) << endl;

  cout<<" normazlied ratio (Ele+   +   Mu+)/(Ele-  +   Mu-)  diff Xec for Data\t"<<endl;
  cout<<"bin \t Norm(Ele+   +  Mu+)  \   Norm(Ele-  +   Mu-) "<<endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      WMEleMMuM_RD_ComFid_NormDiffXsec->GetBinContent(i) / WPElePMuP_RD_ComFid_NormDiffXsec->GetBinContent(i)<<"\t\t"<<
      endl;
  }

  TH1D* h_ElePRatioPreFSR_norm = (TH1D*)f_EleMCCommonFid_ratio->Get("h1_ElePRatioPreFSR_norm")->Clone("h_ElePRatioPreFSR_norm");
  TH1D* h_EleMRatioPreFSR_norm = (TH1D*)f_EleMCCommonFid_ratio->Get("h1_EleMRatioPreFSR_norm")->Clone("h_EleMRatioPreFSR_norm");
  TH1D *WPEleP_RD_Fid_NormDiffXsec=new TH1D("WPEleP_RD_Fid_NormDiffXsec","WPEleP_RD_Fid_NormDiffXsec",nBins-1,WptBins_LinScale); 
  TH1D *WMEleM_RD_Fid_NormDiffXsec=new TH1D("WMEleM_RD_Fid_NormDiffXsec","WMEleM_RD_Fid_NormDiffXsec",nBins-1,WptBins_LinScale); 


  cout<<" "<<endl;
  cout<<"normalizing  ONLY  Ele+  and Ele- for Data\t"<<endl;
  cout<<"bin \t Ele+    \t    Ele- "<<endl;
  cout << fixed << setprecision(10) << endl;
  for (int i(1); i<13 ; i++)
  {
    WPEleP_RD_Fid_NormDiffXsec->SetBinContent(i,
	hWpToEle_RD_yield->GetBinContent(i)/(hWpToEle_RD_yield->Integral()*hWptBins_LinScale->GetXaxis()->GetBinWidth(i)));
    WMEleM_RD_Fid_NormDiffXsec->SetBinContent(i,
	hWmToEle_RD_yield->GetBinContent(i)/(hWmToEle_RD_yield->Integral()*hWptBins_LinScale->GetXaxis()->GetBinWidth(i)));
    
    
    cout<<i<<"\t"<<
      WPEleP_RD_Fid_NormDiffXsec->GetBinContent(i)<<"\t\t"<<
      WMEleM_RD_Fid_NormDiffXsec->GetBinContent(i)<<"\t\t"<<
      endl;
  }

  cout<<"Total Ele+  and   Ele- "<<endl;
  cout<<hWpToEle_RD_yield->Integral() <<endl;
  cout<<hWmToEle_RD_yield->Integral() <<endl;
  cout << fixed << setprecision(3) << endl;
  cout<<" "<<endl;

   TH1D* hWpToEleP_RD_NormDiffXsec_CommonFid;
   hWpToEleP_RD_NormDiffXsec_CommonFid = (TH1D*)WPEleP_RD_Fid_NormDiffXsec->Clone("hWpToEleP_RD_NormDiffXsec_CommonFid");
   hWpToEleP_RD_NormDiffXsec_CommonFid->Multiply(h_ElePRatioPreFSR_norm);
   TH1D* hWmToEleM_RD_NormDiffXsec_CommonFid;
   hWmToEleM_RD_NormDiffXsec_CommonFid = (TH1D*)WMEleM_RD_Fid_NormDiffXsec->Clone("hWmToEleM_RD_NormDiffXsec_CommonFid");
   hWmToEleM_RD_NormDiffXsec_CommonFid->Multiply(h_EleMRatioPreFSR_norm);


  cout<<"Apply commonFid ratio on normalized  Ele+  and Ele- for Data\t"<<endl;
  cout<<"bin \t Ele+    \t    Ele- "<<endl;
  cout << fixed << setprecision(10) << endl;
  for (int i(1); i<13 ; i++)
  {
    cout<<i<<"\t"<<
      //h_ElePRatioPreFSR_norm->GetBinContent(i)<<"\t\t"<<
      //h_EleMRatioPreFSR_norm->GetBinContent(i)<<"\t\t"<<
      hWpToEleP_RD_NormDiffXsec_CommonFid->GetBinContent(i)<<"\t\t"<<
      hWmToEleM_RD_NormDiffXsec_CommonFid->GetBinContent(i)<<"\t\t"<<
      endl;
  }
  cout << fixed << setprecision(3) << endl;
  cout<<" "<<endl;
