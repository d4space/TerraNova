...
{
  gBenchmark->Start("fitWMuMetMtModRayleighSimult_NNLO_PASformat");
...
aqcdMsigma2[ipt]->setVal(2.69409);
    ...
    plotMet->GetLegend()->AddEntry(hDummyData,"Data","PLE");
    ...
    plotAntiMet->GetLegend()->AddEntry(hDummyData,"Data","PLE");
    ...
    //plotMetp->SetLegend(0.68,0.50,0.93,0.70);
    plotMetp->SetLegend(0.63,0.48,0.93,0.70);
    plotMetp->GetLegend()->AddEntry(hDummyData," #font[42]{Data}","PLE");
    ...
    //plotMetp->AddTextBox("18.4 pb^{-1} (8 TeV)",0.63,0.92,0.95,0.99,0);
    
    //plotMetp->AddTextBox(CMStext,0.77,0.80,0.87,0.88,0);
   ...
   // plotMetp->AddTextBox(Preliminarytext,0.24,0.91,0.40,0.96,0);
    ...
    //plotAntiMetp->SetLegend(0.68,0.50,0.93,0.70);
    ...
    plotAntiMetp->GetLegend()->AddEntry(hDummyData," #font[42]{Data}","PLE");
    ...
    //plotMetm->SetLegend(0.68,0.50,0.93,0.70);
    plotMetm->SetLegend(0.63,0.48,0.93,0.70);
    plotMetm->GetLegend()->AddEntry(hDummyData," #font[42]{Data}","PLE");
    ...
    plotAntiMetm->GetLegend()->AddEntry(hDummyData," #font[42]{Data}","PLE");
    ...
    if (ipt>0){
      ...
      W_Err  += nSig[ipt]->getPropagatedError(*fitRes[ipt]);
      Wp_Err += nSigp[ipt]->getPropagatedError(*fitResp[ipt]);
      Wm_Err += nSigm[ipt]->getPropagatedError(*fitResm[ipt]);
  ...
  allyields  <<"Tot. Signal: "<<W_TotalEvents<<"+/-"<<W_Err/13<<" +/- " <<sqrt(W_TotalEvents)<<" (stat.)"<<endl;
...
//Inclusive W pt distribution
  ...
  plotWptDiff=new CPlot(histName,"","p_{T}^{W} [GeV]","(Data-Fit)/#sigma_{Data}");
  ...
  plotWptDiffLog=new CPlot(histName,"","p_{T}^{W} [GeV]","(Data-Fit)/#sigma_{Data}");
...
//W plus pt distribution
  ...
  plotWpt_p->AddToStack(hWToTauNuP,"#font[42]{W#rightarrow#tau#nu}",kCyan-9,kCyan-6);
  plotWpt_p->AddToStack(hDYToMuMuP,"#font[42]{Z/#gamma^{*}#rightarrow #mu#mu}",fillcolorEWK,linecolorEWK);
  plotWpt_p->AddToStack(hQCDWPpt,"#font[42]{QCD}",fillcolorQCD,linecolorQCD);
  plotWpt_p->AddToStack(hSigWPpt,"#font[42]{W^{+}#rightarrow #mu^{+}#nu}",fillcolorW,linecolorW);
  plotWpt_p->AddHist1D(hdataWPpt,"#font[42]{Data}","E");
  ...
  plotWptDiff_p=new CPlot(histName,"","p_{T}^{W} [GeV]","(Data-Fit)/#sigma_{Data}");
  ...
  for (int i=1; i<=hSigWPpt->GetNbinsX(); i++)
  {
    ...
    xEWKp=  nEWKp[i]->getPropagatedError(*fitResp[i]) ;
...
//W minus pt distribution
  ...
  plotWMptLog->AddToStack(hDYToMuMuLogM,"#font[42]{Z/#gamma^{*}#rightarrow #mu#mu}",fillcolorEWK,linecolorEWK);
  ...
  plotWMptLog->SetYRange(0.20,1.4*(hWptMC_m->GetMaximum()));
  plotWMptLog->SetLogx();
  plotWMptLog->SetLogy();
  ...
  //plotWMptDiffLog->AddLine(0,-0.05,600,-0.05,kBlack,3);
  plotWMptDiffLog->AddLine(0, 1.2,600, 1.2,kBlack,3);
...
TH1D *makeDiffHistWptLog(TH1D* hData, TH1D* hFit, const TString name)
{
  ...
  for(int ibin=1; ibin<=hData->GetNbinsX(); ibin++) {
    ...
    cout<<ibin<<
      ...
      "\tData error\t"<<errData<<
      "\tFit\t"<<hFit->GetBinContent(ibin)<<
      "\tFit error\t"<<errFit<<
      "\t diff\t"<<diff<<
      "\t errDiff\t"<<errDiff<<
      "\t errDiff in%\t"<<errDiff/diff*100<<
      endl;
  }
